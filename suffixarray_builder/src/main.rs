...
fn main() -> Result<(), Box<dyn Error>>{
    ...
    let data = get_proteins_from_database_file(&database_file, &*taxon_id_calculator)?;
    let data = &data.input_string; // shadow "data" to force the unused fields being freed
    let sa = build_sa(data, &construction_algorithm, sample_rate)?;
