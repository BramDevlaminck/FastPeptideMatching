...
use suffixarray::suffix_to_protein_index::SuffixToProteinIndex;

pub struct Searcher {
    sa: Vec<i64>,
    pub sample_rate: u8,
    suffix_index_to_protein: SparseSuffixToProtein,
    proteins: Proteins,
...
impl Searcher {
    ...
    pub fn new(sa: Vec<i64>, sample_rate: u8, suffix_index_to_protein: SparseSuffixToProtein, proteins: Proteins, taxon_id_calculator:TaxonIdCalculator) -> Self {
        Self {
            sa,
            sample_rate,
    ...
    pub fn retrieve_proteins(&self, suffixes: &Vec<i64>) -> Vec<&Protein> {
        ...
        for &suffix in suffixes {
            ...
            if protein_index != u32::MAX { // TODO: this was "!protein_index.is_null()"
    ...
    #[inline]
    pub fn retrieve_taxon_id(&self, proteins: &[&Protein]) -> Option<usize> {
        let taxon_ids: Vec<usize> = proteins.iter().map(|prot| prot.id).collect();
    ...
    pub fn search_taxon_id(&self, search_string: &[u8]) -> Option<usize> {
